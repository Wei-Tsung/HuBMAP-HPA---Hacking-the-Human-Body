...
#外部預訓練模型來源Input folder is read-only
import os,gc
import sys
...
#self.register_buffer('name', Tensor)的操作，
# 此用法是定義一组參數，該參數在模型訓練時不會更新
# （即调用 optimizer.step() 該組參數不會變化，只可人為地改變他們的值）
# 但是保存模型時，該組參數又作為模型參數被保存。
